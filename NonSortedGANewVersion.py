...
from MakeNewPopulationVersion2 import *
import csv
...
ParetoOptimalFront={}
class GA:
    def NonSortedGA(self,walkforward_number,MaxIndividualsInGen,MaxGen,MaxIndividuals):
        StoreParetoID={}
        ...
        PerfMeasuresObject=PerfMeasures()
        MakePopObject=MakePop()
        FastSortObject=Sort()
        Pt={}
        logging.info("Current Time %s", str(datetime.now()))
        for num in range(1,MaxIndividualsInGen+1):
            PerfM=PerfMeasuresObject.CalculatePerformanceMeasures(num,walkforward_number,gv.stock_number)
        ...
        for gen in range(1,min(MaxGen,MaxIndividuals/MaxIndividualsInGen)):
            ...
            F=FastSortObject.FastNonDominatedSort(Pt) #Create Fronts From Population
            ...
            for key in F[1].keys():
                F_Reporting=PerfMeasuresObject.CalculatePerformanceMeasuresReporting(key,walkforward_number,gv.stock_number)
            ...
            Q=MakePopObject.MakeNewPopulation(F,Pt,i,gen,MaxIndividualsInGen,lengOfFront1and2)
            Qt={}
            counter=0
            for num in Q:
                PerfM=PerfMeasuresObject.CalculatePerformanceMeasures(num,walkforward_number,gv.stock_number)
            ...
            if(self.CheckConvergenceOfPopulation(F,gen)==0):
    ...
    def CheckConvergenceOfPopulation(self, F,gen):
        NetPL_CurrentGen=0.0
        TotalTrades_CurrentGen=0
